...
import numpy as np
from fsa_methods import mixup, pha_unwrapping
...
class embed_net(nn.Module):
    def __init__(self,  class_num, no_local='on', gm_pool='on', arch='resnet50', fsa_method='None'):
        super(embed_net, self).__init__()

        self.thermal_module = thermal_module(arch=arch)
        self.visible_module = visible_module(arch=arch)
        # self.enhance_module = enhance_module(arch=arch)
