...
def extract_gall_feat(gall_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            feat_pool_1, feat_fc_1 = net(flip_input, flip_input, flip_input, test_mode[0])
...
def extract_query_feat(query_loader):
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            ...
            flip_input = Variable(flip_input.cuda())
            feat_pool_1, feat_fc_1 = net(flip_input, flip_input, flip_input, test_mode[1])
...
if dataset == 'sysu':
    ...
    for trial in range(10):
        test_trial = trial +1
        # model_path = checkpoint_path +  args.resume
        model_path = checkpoint_path + 'regdb_adp_joint_co_nog_ch_nog_sq1_aug_G_erase_0.5_p2_n8_lr_0.1_seed_0_fsa_method_FSA_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
